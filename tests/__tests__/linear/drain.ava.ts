...
test('Non-manager call drain methods', async (t) => {
...
test('drain constraints', async (t) => {
...
test('drain unstake and withdraw', async (t) => {
  const { contract, root, owner, alice, bob } = t.context;
  const manager = alice;
  await setManager(root, contract, owner, manager);
...
test('drain unstake: get_account fails', async (t) => {
  const { contract, root, owner, alice, bob } = t.context;
  const manager = alice;
  await setManager(root, contract, owner, manager);
