...
workspace.test('Lockup account stake', async (test, { root, contract, alice, owner }) => {
  const lockupAccount = await createLockupAccount(root, alice);
  const lockedAmount: string = await lockupAccount.view(
    'get_locked_amount'
  );
  ...
  const poolId = await lockupAccount.view(
  ...
  const lockAccountNewBalance: string = await lockupAccount.view('get_known_deposited_balance');
