...
// --- Base Staking internal methods ---
impl LiquidStakingContract {
    pub(crate) fn internal_base_deposit(&mut self, amount: Balance) {
        self.assert_running();

        let account_id = env::predecessor_account_id();
        let mut account = self.internal_get_base_account(&account_id);
        account.unstaked += amount;
    ...
    pub(crate) fn internal_base_withdraw(&mut self, amount: Balance) {
        self.assert_running();

        let account_id = env::predecessor_account_id();
        self.assert_can_base_withdraw(&account_id, amount);
    ...
    pub(crate) fn internal_base_stake(&mut self, amount: Balance) {
        self.assert_running();
    ...
    pub(crate) fn internal_get_base_account(&self, account_id: &AccountId) -> Account {
        self.base_stake_accounts.get(account_id).unwrap_or_default()
    }

    /// Inner method to save the given base account for a given account ID.
    pub(crate) fn internal_save_base_account(&mut self, account_id: &AccountId, account: &Account) {
        self.base_stake_accounts.insert(account_id, account);
    }
}
