...
workspace.test('sync balance failure after stake/unstake', async (test, { root, contract, alice, owner }) => {
...
workspace.test('sync balance after stake/unstake', async (test, { root, contract, alice, owner }) => {
  const assertValidator = assertValidatorAmountHelper(test, contract, owner);
  const v1 = await createStakingPool(root, 'v1');
  const v2 = await createStakingPool(root, 'v2');
